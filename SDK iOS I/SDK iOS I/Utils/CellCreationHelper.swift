...
class CellCreationHelper {
    ...
    static func cellForProperty(tableView: UITableView, indexPath: IndexPath, entity: EntityValue, property: Property, value: String, editingIsAllowed: Bool = false, isAlternativeInputEnabled: Bool = false, alternativeButtonImage: UIImage? = nil, didSelectAlternativeButtonHandler: (() -> FUIFormTableViewController)? = nil, changeHandler: @escaping ((String) -> Bool)) -> UITableViewCell {
        ...
        if property.dataType.isBasic {
            ...
            if !property.isKey || entity.isNew {
                ...
                if property.dataType.code == DataType.boolean {
                    ...
                    cell = formCellWithEditableContent(tableView: tableView, indexPath: indexPath, property: property, with: value, editingIsAllowed: editingIsAllowed, isAlternativeInputEnabled: isAlternativeInputEnabled, alternativeButtonImage: alternativeButtonImage, didSelectAlternativeButtonHandler: didSelectAlternativeButtonHandler, changeHandler: changeHandler)
                }
            } else {
    ...
    private static func formCellWithEditableContent(tableView: UITableView, indexPath: IndexPath, property: Property, with value: String, editingIsAllowed: Bool = true, isAlternativeInputEnabled: Bool = false, alternativeButtonImage: UIImage? = nil, didSelectAlternativeButtonHandler: (() -> FUIFormTableViewController)? = nil, changeHandler: @escaping ((String) -> Bool)) -> FUITextFieldFormCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: FUITextFieldFormCell.reuseIdentifier, for: indexPath) as! FUITextFieldFormCell
        ...
        cell.isAlternativeInputEnabled = isAlternativeInputEnabled
        if let buttonImage = alternativeButtonImage {
            cell.alternativeInputButton.setImage(buttonImage.withRenderingMode(.alwaysTemplate), for: .normal)
        }
        ...
        if let didSelectAlternativeButtonHandler = didSelectAlternativeButtonHandler {
            cell.alternativeInputButton.didSelectHandler = { _ in
                let fuiFormTableViewController = didSelectAlternativeButtonHandler()
                if let datePickerViewController = fuiFormTableViewController as? DatePickerViewController {
                    datePickerViewController.dateValue = cell.value
                    datePickerViewController.onSave = { data in
                        cell.value = data
                        if let setChangeHandler = cell.onChangeHandler {
                            setChangeHandler(cell.value)
                        }
                    }
                }
            }
