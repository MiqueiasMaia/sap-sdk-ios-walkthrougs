...
public class ESPMContainerWidgetController: WidgetControlling {
    ...
    public func configure(with cipher: Ciphering) throws {
        do {
            ...
            dataServiceProvider = DefaultAuxiliaryDataServiceProvider(provider: communicationProvider, with: cipher)
            dataRequestManager = AuxiliaryDataRequestManager(provider: communicationProvider)
        } catch {
            throw error
        }
    }
    ...
    public func initiateDataRequest(dataRequest: AuxiliaryDataRequest<WidgetDataKey>) throws {
        try dataRequestManager.putDataRequest(dataRequest: dataRequest)
    }

    public func identifyDataRequest() throws -> WidgetDataKey? {
        let dataContainer: AuxiliaryDataRequest<WidgetDataKey>? = try dataRequestManager.getDataRequest()
    ...
    public func completeDataRequest() throws {
        let dataContainer: AuxiliaryDataRequest<WidgetDataKey>? = try dataRequestManager.getDataRequest()
        if let dataContainerNotNil = dataContainer {
            try dataRequestManager.completeDataRequest(dataRequest: dataContainerNotNil)
        }
    }
    ...
    open func put(list: [EntityValue], forKey key: String) throws {
        ...
        if let encodedEntityValue = try? encoder.encode(list) {
            ...
            try dataServiceProvider.put(auxDataContainer: dataContainer, for: keyContainer)
        }
    }

    func getKeyContainer(for entity: String) -> AuxiliaryKeyContainer<WidgetDataKey> {
        let dataKey = WidgetDataKey(destinationName: ODataContainerType.eSPMContainer.description, entityName: entity)
    ...
    open func fetchPurchaseOrderItems() throws -> [ESPMContainerFmwk.PurchaseOrderItem] {
        ...
        let dataContainer: AuxiliaryDataContainer<Data> = try dataServiceProvider.get(auxKeyContainer: keyContainer)
